...
How it works:
- This script works same as 'test-scalability.sh' except, instead of opening terminals and displaying logs on terminal, this redirects them to files located at '/tmp/$USER/distributed/HW2-PC'.
...
- For ex, your 'machine_list' file has 20 hosts and when you run './test-scalability-online.sh 5', this script starts 20*5 = 100 Client processes.
   - Each machine starts <number_of_client_on_each_machine> Client processes and their outputs will be redirected to '/tmp/$USER/distributed/HW2-PC' directory. You don't have to make this directory. If it doesn't exist, it will be created. Each file in this directory is output of a Client process.
...
SCRIPT="cd $CLASSES;
java -cp . distributed.scaling.client.Client [server-host] [server-listening-port] > "
...
do
	...
	do
		echo 'logging into '$i
		FOLDER="/tmp/$USER/distributed/HW2-PC"
		FILE="$FOLDER/$j"
